...
#include "NativeRegisterContextHaiku_x86_64.h"
...
NativeRegisterContextHaiku::CreateHostNativeRegisterContextHaiku(
  ...
  return new NativeRegisterContextHaiku_x86_64(target_arch, native_thread);
}

// NativeRegisterContextHaiku_x86_64 members.

static RegisterInfoInterface *
CreateRegisterInfoInterface(const ArchSpec &target_arch) {
  if (HostInfo::GetArchitecture().GetAddressByteSize() == 4) {
    // 32-bit hosts run with a RegisterContextHaiku_i386 context.
    return new RegisterContextHaiku_i386(target_arch);
  } else {
    assert((HostInfo::GetArchitecture().GetAddressByteSize() == 8) &&
           "Register setting path assumes this is a 64-bit host");
    // X86_64 hosts know how to work with 64-bit and 32-bit EXEs using the
    // x86_64 register context.
    return new RegisterContextHaiku_x86_64(target_arch);
  }
}

NativeRegisterContextHaiku_x86_64::NativeRegisterContextHaiku_x86_64(
    const ArchSpec &target_arch, NativeThreadProtocol &native_thread)
...
uint32_t NativeRegisterContextHaiku_x86_64::GetRegisterSetCount() const {
  return k_num_register_sets;
}

const RegisterSet *
NativeRegisterContextHaiku_x86_64::GetRegisterSet(uint32_t set_index) const {
  switch (GetRegisterInfoInterface().GetTargetArchitecture().GetMachine()) {
  case llvm::Triple::x86:
...
llvm::Optional<NativeRegisterContextHaiku_x86_64::RegSetKind>
NativeRegisterContextHaiku_x86_64::GetSetForNativeRegNum(
...
Status
NativeRegisterContextHaiku_x86_64::ReadRegister(const RegisterInfo *reg_info,
...
Status NativeRegisterContextHaiku_x86_64::WriteRegister(
...
Status NativeRegisterContextHaiku_x86_64::WriteAllRegisterValues(
    const lldb::DataBufferSP &data_sp) {
  Status error;

  if (!data_sp) {
    error.SetErrorStringWithFormat(
        "NativeRegisterContextHaiku_x86_64::%s invalid data_sp provided",
        __FUNCTION__);
    return error;
  }
  ...
  if (data_sp->GetByteSize() != REG_CONTEXT_SIZE) {
    error.SetErrorStringWithFormat(
        "NativeRegisterContextHaiku_x86_64::%s data_sp contained mismatched "
  ...
  if (src == nullptr) {
    error.SetErrorStringWithFormat("NativeRegisterContextHaiku_x86_64::%s "
...
llvm::Error NativeRegisterContextHaiku_x86_64::CopyHardwareWatchpointsFrom(
    NativeRegisterContextHaiku &source) {
  auto &r_source = static_cast<NativeRegisterContextHaiku_x86_64 &>(source);
